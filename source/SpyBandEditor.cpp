...
bool PLUGIN_API SpyBandEditor::open (void* parent, const PlatformType& platformType)
{
	...
	addSlider (kInLevel,          "L Src Level",  12, 106, 46, 17);
...
void SpyBandEditor::refresh ()
{
	...
	//
	// With Interlace on the split is real - Src Level takes the left and
	// Wav Level takes the right - so the labels can say so plainly. With
	// it off both channels are the carrier and Src Level scales both,
	// while Wav Level is scaling the sample slots and not a channel at
	// all, so the prefixes follow the mode rather than lying in one of
	// them. See DEVIATION 6.
	//--------------------------------------------------------------------
	{
		const bool interlaced =
			mController->getParamNormalized (kInterlaced) >= 0.5;

		if (auto* control = mControls[kInLevel])
			static_cast<SpySlider*> (control)->setLabel (
				interlaced ? "L Src Level" : "L+R Src Level");
